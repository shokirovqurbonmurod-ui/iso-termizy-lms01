...
const emptyForm = { title: '', subject: '', group_name: '', description: '', deadline: new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10), coin_reward: 20 };
...
export default function HomeworkPage() {
  ...
  async function load() {
    const [h, g, s, c] = await Promise.all([
      api.get('/homework?limit=500').catch(() => []),
      api.get('/groups').catch(() => []),
      api.get('/students').catch(() => []),
      api.get('/assignment_completions?limit=2000').catch(() => []),
    ]);
    setItems((h || []).sort((a, b) => (b.deadline || '').localeCompare(a.deadline || '')));
    setGroups(g || []); setStudents(s || []);
    setCompletions((c || []).filter((x) => x.item_type === 'homework'));
  }
  useEffect(() => { load(); }, []);

  const filtered = useMemo(() => {
    const base = canManage ? (items || []) : (items || []).filter((i) => i.group_name === user.group_name);
    return groupFilter ? base.filter((i) => i.group_name === groupFilter) : base;
  }, [items, groupFilter, canManage, user.group_name]);
  ...
  async function save() {
    if (!form.title.trim() || !form.group_name) { alert("Vazifa nomi va guruhni kiriting"); return; }
    setSaving(true);
    try {
      await api.post('/homework', { ...form, coin_reward: Math.min(200, Number(form.coin_reward) || 0), submitted: 0, status: 'open' });
  ...
  async function submitMine(item) {
    ...
    if (existing) return;
    try {
      await api.post('/assignment-actions/submit', { item_id: item.id, item_type: 'homework' });
      await load();
    } catch (e) { alert(e.message); }
  }
  ...
  async function approve(item, studentName) {
    ...
    if (student && Number(item.coin_reward) > 0) {
      await api.post('/coins/give', { student_id: student.id, amount: Number(item.coin_reward), reason: `Uy vazifasi: ${item.title}` }).catch(() => {});
    }
    await load();
  }
  ...
  return (
    <div>
      <PageHeader icon={ClipboardList} title="Uy vazifalari" subtitle="Guruhlarga berilgan vazifalar, topshirish va tasdiqlash"
      ...
      {filtered.length === 0 ? <Empty icon={ClipboardList} title="Vazifa yo'q" /> : (
      ...
      <Modal open={modal} title="Yangi uy vazifasi" onClose={() => setModal(false)}
        footer={<>
      ...
      >
        ...
        <textarea className="input !py-2.5 mb-4" rows={2} placeholder="Vazifa tavsifi..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
