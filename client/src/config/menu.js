...
import { Presentation, ClipboardList, ListChecks, Coins, Contact, Receipt, Percent, MessageSquare, ScanFace, Bot } from 'lucide-react';
...
export const MENU = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'resource-files', label: 'Resurs & Fayllar', icon: Library, kind: 'tabs', roles: null, tabs: [
        { key: 'resource-center', label: 'Resurs markazi', icon: Library, kind: 'resource', resource: 'resource_center', roles: null },
        { key: 'files', label: 'Fayl boshqaruvchi', icon: FileText, kind: 'resource', resource: 'file_manager', roles: null },
      ] },
      { key: 'qr-checkin', label: 'QR & Check-in', icon: Smartphone, kind: 'tabs', roles: null, tabs: [
        { key: 'qr-m', label: 'QR kodlar', icon: Smartphone, kind: 'resource', resource: 'qr_codes', roles: null },
        { key: 'check-in', label: 'Check-in', icon: Smartphone, kind: 'resource', resource: 'check_ins', roles: null },
        { key: 'booking-m', label: 'Booking', icon: Calendar, kind: 'resource', resource: 'booking_slots', roles: null },
      ] },
      { key: 'marketing-content', label: 'Marketing kontent', icon: MessageCircle, kind: 'tabs', roles: ['director','super_admin','admin','marketing','smm'], tabs: [
        { key: 'social-media-posts', label: 'Ijtimoiy tarmoq', icon: MessageCircle, kind: 'resource', resource: 'social_media_posts', roles: null },
        { key: 'content-calendar', label: 'Kontent kalendari', icon: Calendar, kind: 'resource', resource: 'content_calendar', roles: null },
        { key: 'brand-assets', label: 'Brend aktivlari', icon: Star, kind: 'resource', resource: 'brand_assets', roles: null },
        { key: 'newsletter', label: 'Newsletter', icon: Send, kind: 'resource', resource: 'newsletter_log', roles: null },
      ] },
      ...
      { key: 'building-safety', label: 'Bino & Xavfsizlik', icon: ShieldCheck, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        ...
        { key: 'safety-inspections', label: 'Xavfsizlik tekshiruvi', icon: ShieldCheck, kind: 'resource', resource: 'safety_inspections', roles: ADMIN_ONLY },
        { key: 'fire-drill-log', label: "Yong'in mashqi", icon: Flame, kind: 'resource', resource: 'fire_drill_log', roles: ADMIN_ONLY },
        { key: 'evacuation-plans', label: 'Evakuatsiya rejasi', icon: Map, kind: 'resource', resource: 'evacuation_plans', roles: ADMIN_ONLY },
        { key: 'first-aid', label: 'Birinchi yordam', icon: Star, kind: 'firstaidlog', roles: ADMIN_ONLY },
        { key: 'first-aid-supplies', label: 'Tibbiy jihozlar', icon: Star, kind: 'resource', resource: 'first_aid_supplies', roles: ADMIN_ONLY },
      ] },
      ...
      { key: 'procurement-vendor', label: 'Xarid & Yetkazuvchilar', icon: ShoppingBag, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        ...
        { key: 'vendor-management', label: 'Yetkazuvchilar', icon: Building2, kind: 'resource', resource: 'vendor_management', roles: ADMIN_ONLY },
      ...
      { key: 'it-licenses', label: 'IT jihozlar & Litsenziyalar', icon: KeyRound, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'it-assets-2', label: 'IT jihozlar', icon: Smartphone, kind: 'resource', resource: 'it_assets', roles: ADMIN_ONLY },
        { key: 'software-licenses-2', label: 'Litsenziyalar', icon: KeyRound, kind: 'resource', resource: 'software_licenses', roles: ADMIN_ONLY },
        { key: 'keys', label: 'Kalit boshqaruvi', icon: KeyRound, kind: 'keymanagement', roles: ADMIN_ONLY },
      ] },
      { key: 'network-monitoring-group', label: 'Tarmoq & Monitoring', icon: Gauge, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'network-monitoring', label: 'Tarmoq', icon: BarChart3, kind: 'resource', resource: 'network_monitoring', roles: ADMIN_ONLY },
        { key: 'performance-metrics', label: 'Ishlash', icon: Gauge, kind: 'resource', resource: 'performance_metrics', roles: ADMIN_ONLY },
        { key: 'sla-tracking', label: 'SLA', icon: Clock, kind: 'resource', resource: 'sla_tracking', roles: ADMIN_ONLY },
        { key: 'uptime-monitor', label: 'Uptime', icon: Gauge, kind: 'resource', resource: 'uptime_monitor', roles: ADMIN_ONLY },
        { key: 'app-analytics', label: 'Ilova analitikasi', icon: BarChart3, kind: 'resource', resource: 'app_analytics', roles: ADMIN_ONLY },
      ] },
      { key: 'bugs', label: 'Xatolar & Buglar', icon: AlertTriangle, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'bug-reports', label: 'Xato hisobotlari', icon: AlertTriangle, kind: 'resource', resource: 'bug_reports', roles: ADMIN_ONLY },
        { key: 'feature-requests', label: "Funksiya so'rovlari", icon: Lightbulb, kind: 'resource', resource: 'feature_requests', roles: ADMIN_ONLY },
        { key: 'user-feedback', label: 'Foydalanuvchi fikrlari', icon: MessageSquare, kind: 'feedback', roles: ADMIN_ONLY },
      ] },
      { key: 'api-integrations', label: 'API & Integratsiyalar', icon: BarChart3, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'api-usage-2', label: 'API foydalanish', icon: BarChart3, kind: 'resource', resource: 'api_usage', roles: ADMIN_ONLY },
        { key: 'webhook-log-2', label: 'Webhook', icon: Send, kind: 'resource', resource: 'webhook_log', roles: ADMIN_ONLY },
        { key: 'anticheat', label: 'Anti-cheat', icon: ShieldCheck, kind: 'resource', resource: 'anti_cheat_log', roles: ADMIN_ONLY },
      ] },
      { key: 'import-export', label: 'Import & Export', icon: DatabaseBackup, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'data-imports', label: "Ma'lumot import", icon: DatabaseBackup, kind: 'resource', resource: 'data_imports', roles: ADMIN_ONLY },
        { key: 'data-exports', label: "Ma'lumot eksport", icon: DatabaseBackup, kind: 'resource', resource: 'data_exports', roles: ADMIN_ONLY },
      ] },
      { key: 'migration-webhook', label: 'Migratsiya & Webhook', icon: Repeat, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'migration-log', label: 'Migratsiya', icon: Repeat, kind: 'resource', resource: 'migration_log', roles: ADMIN_ONLY },
        { key: 'webhook-log', label: 'Webhook logi', icon: Send, kind: 'resource', resource: 'webhook_log', roles: ADMIN_ONLY },
      ] },
      { key: 'backup', label: 'Backup & Tiklash', icon: DatabaseBackup, kind: 'resource', resource: 'backups', roles: ['director','super_admin','it_admin'] },
      { key: 'system-updates', label: 'Tizim yangilanishlari', icon: Settings, kind: 'resource', resource: 'system_updates', roles: ADMIN_ONLY },
      { key: 'templates', label: 'Shablonlar markazi', icon: FileText, kind: 'tabs', roles: ADMIN_ONLY, tabs: [
        { key: 'notification-templates', label: 'Xabarnoma', icon: Bell, kind: 'resource', resource: 'notification_templates', roles: ADMIN_ONLY },
        { key: 'email-templates', label: 'Email', icon: Send, kind: 'resource', resource: 'email_templates', roles: ADMIN_ONLY },
        { key: 'sms-templates', label: 'SMS', icon: Send, kind: 'resource', resource: 'sms_templates', roles: ADMIN_ONLY },
        { key: 'report-templates', label: 'Hisobot', icon: FileText, kind: 'resource', resource: 'report_templates', roles: ADMIN_ONLY },
        { key: 'document-templates', label: 'Hujjat', icon: FileText, kind: 'resource', resource: 'document_templates', roles: ADMIN_ONLY },
      ] },
      ...
      { key: 'misc-legacy', label: 'Boshqa funksiyalar', icon: Package, kind: 'tabs', roles: null, tabs: [
        ...
        { key: 'weekly-tasks', label: 'Haftalik vazifalar', icon: Target, kind: 'resource', resource: 'weekly_tasks', roles: null },
        { key: 'quiz-questions', label: "O'yin savollari", icon: ClipboardList, kind: 'resource', resource: 'quiz_questions', roles: null },
        { key: 'mystery-box', label: 'Mystery box', icon: Package, kind: 'mysterybox', roles: null },
        { key: 'mystery-box-log', label: 'Mystery box tarixi', icon: Package, kind: 'resource', resource: 'mystery_box', roles: ['founder','director','super_admin','branch_manager','admin','academic_manager'] },
        ...
        { key: 'cleaning', label: 'Tozalash jadvali', icon: Calendar, kind: 'cleaningschedule', roles: null },
        { key: 'meetings-n', label: "Yig'ilish qarorlari", icon: FileText, kind: 'meetings', roles: null },
        { key: 'int-ann', label: "Ichki e'lonlar", icon: Bell, kind: 'internalannouncements', roles: null },
        { key: 'evt-tickets', label: 'Tadbir biletlari', icon: Receipt, kind: 'resource', resource: 'event_tickets', roles: null },
        { key: 'franchise', label: 'Franshiza', icon: Building2, kind: 'franchisedashboard', roles: null },
        { key: 'roleplay', label: "Rol o'yini", icon: Target, kind: 'resource', resource: 'role_play_scenarios', roles: null },
        { key: 'diary-m', label: 'Shaxsiy kundalik', icon: NotebookPen, kind: 'resource', resource: 'diary', roles: null },
        ...
        { key: 'avatar-shop-m', label: 'Avatar shop', icon: Star, kind: 'avatarshoppage', roles: null },
        { key: 'birthday-m', label: "Tug'ilgan kun", icon: Gift, kind: 'birthdaybonuspage', roles: null },
        { key: 'seasonal-m', label: 'Mavsumiy tadbirlar', icon: Calendar, kind: 'resource', resource: 'seasonal_events', roles: null },
        { key: 'countdown-m', label: 'Countdown', icon: Clock, kind: 'resource', resource: 'countdown_events', roles: null },
        { key: 'polls-m', label: "Tez so'rovnoma", icon: ClipboardList, kind: 'votingpage', roles: null },
        { key: 'karaoke-menu', label: 'Karaoke', icon: Star, kind: 'resource', resource: 'karaoke', roles: null },
        { key: 'whiteboard-menu', label: 'Whiteboard', icon: Presentation, kind: 'resource', resource: 'whiteboard_sessions', roles: null },
        { key: 'podcast-menu', label: 'Podcast', icon: MessageSquare, kind: 'resource', resource: 'podcast_episodes', roles: null },
        { key: 'voting', label: 'Saylov', icon: ClipboardList, kind: 'votingpage', roles: null },
        { key: 'live-stream', label: 'Jonli efir', icon: Video, kind: 'resource', resource: 'live_streams', roles: null },
        { key: 'story-menu', label: 'Story', icon: Star, kind: 'resource', resource: 'story_posts', roles: null },
        { key: 'virtual-room', label: 'Virtual xona', icon: Building2, kind: 'resource', resource: 'virtual_rooms', roles: null },
        { key: 'daily-puzzle', label: 'Kunlik topishmoq', icon: Target, kind: 'dailypuzzle', roles: null },
        { key: 'boss-fight', label: 'Boss fight', icon: Trophy, kind: 'bossfight', roles: null },
        { key: 'fitness', label: 'Fitness challenge', icon: TrendingUp, kind: 'resource', resource: 'fitness_challenge', roles: null },
        { key: 'stocks', label: "Aksiya o'yini", icon: TrendingUp, kind: 'resource', resource: 'stock_game', roles: null },
        { key: 'voice-msg', label: 'Ovozli xabar', icon: MessageSquare, kind: 'resource', resource: 'chat_voice', roles: null },
        { key: 'social-share', label: 'Ijtimoiy ulashish', icon: Send, kind: 'resource', resource: 'social_shares', roles: null },
        { key: 'theme', label: 'Tema sozlash', icon: Settings, kind: 'resource', resource: 'theme_settings', roles: null },
        { key: 'calc', label: 'Kalkulator', icon: Calculator, kind: 'resource', resource: 'calculator_history', roles: null },
        { key: 'study_groups', label: 'Study guruhlar', icon: Users, kind: 'resource', resource: 'study_groups', roles: null },
        { key: 'subscription-boxes', label: 'Obuna qutilari', icon: Gift, kind: 'resource', resource: 'subscription_boxes', roles: null },
        ...
        { key: 'scholarship-applications', label: 'Stipendiya arizalari', icon: Gift, kind: 'scholarshipapplications', roles: null },
        { key: 'revenue-forecast', label: 'Daromad bashorati', icon: TrendingUp, kind: 'resource', resource: 'revenue_forecast', roles: null },
        { key: 'expense-forecast', label: 'Xarajat bashorati', icon: Wallet, kind: 'resource', resource: 'expense_forecast', roles: null },
        { key: 'overtime-log', label: "Qo'shimcha ish", icon: Clock, kind: 'resource', resource: 'overtime_log', roles: null },
        { key: 'travel-requests', label: 'Xizmat safari', icon: Send, kind: 'resource', resource: 'travel_requests', roles: null },
        { key: 'petty-cash', label: 'Mayda xarajatlar', icon: PiggyBank, kind: 'resource', resource: 'petty_cash', roles: null },
